...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False, **kwargs):
    if vanilla_generation:
        if isinstance(prompts, str):
            prompts, targets = [prompts, ], [targets, ]
        # # 处理多跳问题的提示词
        # if "portability_key" in kwargs and kwargs["portability_key"] == "multihop":
        #     multihop_prompt = "Please carefully analyze the following question, which may be a multi-step reasoning multi-hop question. Please think step by step, ensure each reasoning step is correct, and synthesize all relevant information to reach a final answer. Only output the answer without any additional explanations or text.\n\n"
        #     prompts = [multihop_prompt + prompt for prompt in prompts]
        results = []
        # formatted_prompts = ["Question: " + p + "\nOnly output the final answer.\nAnswer: " for p in prompts]
        
        # vicuna-7b
        formatted_prompts = [
            "A chat between a curious user and an artificial intelligence assistant. The assistant gives concise, direct, and short answers to the user's questions without any preamble or repetition. "
            f"USER: Question: {p}\n"
            f"Answer the question directly with only the entity name or the key fact.\n"
            f"ASSISTANT:\n"
            for p in prompts
        ]
        for formatted_prompt, prompt, target_new in zip(formatted_prompts, prompts, targets):
        # for prompt, target_new in zip(prompts, targets):
            ...
            gen_token = model.generate(
                input_ids=prompt_tok['input_ids'],
                attention_mask=prompt_tok['attention_mask'],
                max_new_tokens=32, # len(target_new_tokens)
                pad_token_id=tok.eos_token_id,
                eos_token_id=tok.eos_token_id,
                do_sample=True,             # 开启采样
                temperature=0.1,            # 极低温度，保证事实回答的稳定性
                top_p=0.9,
                # do_sample=False,
                use_cache=True,
                # use_cache=False,
            ...
            format_gen_token = model.generate(
                input_ids=format_prompt_tok['input_ids'],
                attention_mask=format_prompt_tok['attention_mask'],
                max_new_tokens=24, # len(target_new_tokens)
                pad_token_id=tok.eos_token_id,
                eos_token_id=tok.eos_token_id,
                do_sample=True,             # 开启采样
                temperature=0.05,            # 极低温度，保证事实回答的稳定性
                top_p=0.9,
                # do_sample=False,
                # use_cache=True,
                # use_cache=False,
            ...
            elif kwargs['pkey'] == 'single_hop':
                # 提取模型生成的文本
                gen_text = tok.decode(gen_token[0][prompt_tok['input_ids'].shape[1]:], skip_special_tokens=True).lower()
                format_gen_text = tok.decode(format_gen_token[0][format_prompt_tok['input_ids'].shape[1]:], skip_special_tokens=True).lower()
                gen_text = gen_text+ " " + format_gen_text
                # print("gentext:",gen_text)
                # 假设 target_new 传进来的是包含了多个别名的列表，例如 ["Joe Biden", "Joseph Biden"]
                # 如果你数据结构里只有一个字符串，那就把它转成小写
                if isinstance(target_new[0],list):
                    target_new = target_new[0]
                correct_aliases =[t.lower() for t in target_new] if isinstance(target_new, list) else [target_new.lower()]
                
                # 只要生成的句子里包含了任何一个正确答案的别名，就是 1.0，否则 0.0
                is_correct = any(alias in gen_text for alias in correct_aliases)
    ...
    with torch.no_grad():
        ...
        prompt_target_tok = accelerator.prepare(prompt_target_tok)
...
#     # =========================================================================
#     # 🌟 新增拦截层：Internal-RAG 测试期动态上下文生成 (三次调用)
#     # =========================================================================
#     # 对于 locality 测试（通常是无关领域的知识），我们不触发复杂的图谱推理以避免幻觉。
#     # 只针对主评测和 portability 测试启用 Internal-RAG。
#     if not locality:
#         is_str = isinstance(prompts, str)
#         prompts_list = [prompts] if is_str else prompts
#         final_prompts =[]
        
#         model.eval() # 确保处于推理模式
        
#         for q in prompts_list:
# #             # Step 1: 诱导模型默写内部图谱知识 (A1)
# #             retrieval_prompt = f"Retrieve the structural knowledge from your internal memory related to the question: \"{q}\""
# #             retrieval_inputs = tok(retrieval_prompt, return_tensors="pt").to(f"cuda:{device}")
# #             with torch.no_grad():
# #                 a1_ids = model.generate(
# #                     input_ids=retrieval_inputs['input_ids'],
# #                     attention_mask=retrieval_inputs.get('attention_mask'),
# #                     max_new_tokens=100, # 可根据图谱大小调整
# #                     do_sample=False,
# #                     use_cache=True,
# #                     pad_token_id=tok.eos_token_id
# #                 )
# #             # 截取新生成的 token 作为 A1
# #             a1_new_ids = a1_ids[0][retrieval_inputs['input_ids'].shape[1]:]
# #             A1 = tok.decode(a1_new_ids, skip_special_tokens=True).strip()

            
#             # Step 2: 诱导模型生成推理链 (A2)
#             reasoning_prompt = f"Please perform multi-hop reasoning to solve this. Identify the initial entity, and then logically deduce the connecting entities step-by-step until you reach the final target. What is the logical chain to answer the question: \"{q}\"?"
#             reasoning_inputs = tok(reasoning_prompt, return_tensors="pt").to(f"cuda:{device}")
#             with torch.no_grad():
#                 a2_ids = model.generate(
...
#                 )
#             # 截取新生成的 token 作为 A2
#             a2_new_ids = a2_ids[0][reasoning_inputs['input_ids'].shape[1]:]
#             A2 = tok.decode(a2_new_ids, skip_special_tokens=True).strip()

#             # Step 3: 组装终极 Prompt
#             final_prompt = (
#                 f"Context:\n"
# #                 f"Internal Knowledge Graph: {A1}\n"
#                 f"Reasoning: {A2}\n"
#                 f"Question: {q}"
#             )
...
#             # 调试打印，方便你观察模型在测试时自问自答的过程
#             # print(f"\n[Internal-RAG Debug] Q: {q}\n[A1]: {A1}\n[A2]: {A2}\n")

#         # 替换原本的 prompts，无缝衔接 EasyEdit 原生评测逻辑
#         prompts = final_prompts[0] if is_str else final_prompts
#     # =========================================================================

#     # 以下完全保留原始 EasyEdit 的评测逻辑
#     if vanilla_generation:
#         if isinstance(prompts, str):
#             prompts, targets = [prompts, ], [targets, ]
#         results =[]
...
#     with torch.no_grad():
#         # 如果你使用了 accelerate 库包裹，可能需要从外部引入 accelerator
#         # 否则建议将这两行 accelerator.prepare() 注释掉，直接用 model
#         # model = accelerator.prepare(model)
#         # prompt_target_tok = accelerator.prepare(prompt_target_tok)
...
#     # =========================================================================
#     # 🌟 新增拦截层：Internal-RAG 测试期动态上下文生成 (三次调用)
#     # 仅针对非局部性 (locality=False) 且非单字符生成的任务启用
#     # =========================================================================
#     if not locality:
#         is_str = isinstance(prompts, str)
#         prompts_list =[prompts] if is_str else prompts
...
#         for q in prompts_list:
#             # Step 1: 诱导模型默写内部图谱知识 (A1)
#             retrieval_prompt = f"Retrieve the structural knowledge from your internal memory for the question: \"{q}\""
#             retrieval_inputs = tok(retrieval_prompt, return_tensors="pt").to(f"cuda:{device}")
#             with torch.no_grad():
#                 a1_ids = model.generate(
#                     input_ids=retrieval_inputs['input_ids'],
#                     attention_mask=retrieval_inputs.get('attention_mask'),
#                     max_new_tokens=150, # 图谱可能较长，放宽到150
#                     do_sample=False,
#                     pad_token_id=tok.eos_token_id
#                 )
#             a1_new_ids = a1_ids[0][retrieval_inputs['input_ids'].shape[1]:]
...
#             # Step 2: 诱导模型生成推理链 (A2)
#             reasoning_prompt = f"Given the internal knowledge: {A1}\nWhat is the logical chain to answer the question: \"{q}\"?"
#             reasoning_inputs = tok(reasoning_prompt, return_tensors="pt").to(f"cuda:{device}")
#             with torch.no_grad():
#                 a2_ids = model.generate(
#                     input_ids=reasoning_inputs['input_ids'],
...
#     gen_kwargs = {
#         "max_new_tokens": 150,
#         "do_sample": False, # 编辑任务通常使用 greedy search 保证精确性
#         "pad_token_id": tok.eos_token_id,
#         "eos_token_id": tok.eos_token_id,
...
#         for q in prompts_list:
#             # Step 1: Internal Retrieval (使用稍宽的 penalty，因为图谱结构本身可能有重复词)
#             retrieval_prompt = f"Retrieve the structural knowledge from your internal memory for the question: \"{q}\""
#             retrieval_inputs = tok(retrieval_prompt, return_tensors="pt").to(f"cuda:{device}")
#             with torch.no_grad():
#                 a1_ids = model.generate(
#                     input_ids=retrieval_inputs['input_ids'],
#                     attention_mask=retrieval_inputs.get('attention_mask'),
#                     # 适当放宽图谱生成的惩罚，因为 Entity [A] 这种格式本身是重复的
...
#         prompts = final_prompts[0] if is_str else final_prompts
    ...
    #                                     add_generation_prompt=True,
    #                                     tokenize=False)
    # prompt_target =[prompt + ' Provide only the exact target entity. ' + target for prompt, target in zip(prompts,targets)]
    # max_prompt_len = max([len(tok.encode(_)) for _ in prompt_target]) + 1
    # before_padding_side = tok.padding_side
    # tok.padding_side = 'left'
    # prompt_target_tok = tok(
    ...
    #     answers = torch.argmax(logits, dim=-1).squeeze().detach().cpu().numpy().tolist()
    #     labels = prompt_target_tok['input_ids'].squeeze().detach().cpu().numpy().tolist()
    #     answers = slice_list(answers,prompt_len,left=True) if 'slice_list' in globals() else [ans[-len(lbl):] for ans, lbl in zip(answers, prompt_len)] # 防护slice_list缺失
    #     labels = slice_list(labels,prompt_len,left=False) if 'slice_list' in globals() else [lbl[-len(pl):] for lbl, pl in zip(labels, prompt_len)]
