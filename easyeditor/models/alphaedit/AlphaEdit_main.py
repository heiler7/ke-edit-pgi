...
def execute_AlphaEdit(
    ...
    for i in range(len(requests)):
        if isinstance(requests[i]["target_new"], list):
            requests[i]["target_new"] = requests[i]["target_new"][0]
        if isinstance(requests[i]["prompt"], list):
            requests[i]["prompt"] = requests[i]["prompt"][0]
        if isinstance(requests[i]["subject"], list):
            requests[i]["subject"] = requests[i]["subject"][0]
