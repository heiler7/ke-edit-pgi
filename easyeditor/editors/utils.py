        ...
        for key in ["locality", "portability"]:
            if key in all_metrics[0][eval].keys() and all_metrics[0][eval][key] != {}:
                ...
                for lkey in get_all_acc_keys(all_metrics):
                    if lkey == "single_hop_acc":
                        metrics = [ 1 if all(me == 1 for me in metric[eval][key][lkey]) else 0 for metric in all_metrics if lkey in metric[eval][key].keys()]
                    else:
...
def _prepare_requests(prompts: Union[str, List[str]],
    ...
    if 'subject' in kwargs:
        ...
        for prompt_, subject_ in zip(prompts, kwargs['subject']):
            if isinstance(prompt_,list) and isinstance(prompt_,list):
                for prompt_i,subject_i in zip(prompt_,subject_):
                    assert subject_i in prompt_i, print(f'Subject:{subject_i} do not exist in prompt: {prompt_i}')
            else:
    ...
    if 'rephrase_example' in kwargs:
        if isinstance(kwargs['rephrase_example'], str):
            kwargs['rephrase_example'] = [kwargs['rephrase_example'],]

        for i, request in enumerate(requests):
            request.update(
                {
                    'rephrase_example': kwargs['rephrase_example'][i],
                }
            )
